...
- Space and Enter keys switched - Space on right, Enter on left.
  - I find myself normally hitting the Space key with my right thumb. I already have to adjust to the Enter key being on the thumb, so this minimises the number of adjustments I have to make. I can't see any reason why the Enter key should be on the right side. A potential downside I can see is that it makes it slightly harder to type with only the left hand (when my right is on the mouse for example). This is not a big deal. The other potential downside is that I can't type a space and then a symbol without moving my thumb to the UPPER modified. This could be a deal breaker. Another thing is that I can't hit space to navigate down a page when I have my hand on the mouse. But then I have the same problem with the numbers and space...
